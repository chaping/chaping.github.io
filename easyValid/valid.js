...
(function($){
	...
	var rules ={
		...
		'en'         : /^[A-Za-z]+$/,  //英文字母
		'zh'         : /^[\u0391-\uFFE5]+$/,  //中文
		'cellphone'  : /^((\(\d{2,3}\))|(\d{3}\-))?13\d{9}$/, //固定电话
		'telphone'   : /^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/, //手机号
	...
	function init(form,opts){
		var elems = getVerifyElems(form); //表单元素集合
		...
		for(var i=0,len=elems.length;i<len;i++){
			...
			conf.errmsg = $(elems[i]).attr('errmsg') || opts.errmsg; //验证不通过提示信息
			conf.errcls = $(elems[i]).attr('errcls') || opts.errcls; //验证不通过时的类名
			conf.f_errcls = $(elems[i]).attr('f_errcls') || opts.f_errcls; //验证不通过时的表单元素类名
			conf.okmsg = $(elems[i]).attr('okmsg') || opts.okmsg; //验证通过提示信息
			conf.okcls = $(elems[i]).attr('okcls') || opts.okcls; //验证通过时的类名
			conf.f_okcls = $(elems[i]).attr('f_okcls') || opts.f_okcls; //验证通过时的表单元素类名
			conf.tipmsg = $(elems[i]).attr('tipmsg') || opts.tipmsg; //填写提示信息
			conf.tipcls = $(elems[i]).attr('tipcls') || opts.tipcls; //提示填写时的类名
			conf.f_tipcls = $(elems[i]).attr('f_tipcls') || opts.f_tipcls; //提示填写时的表单元素类名
			conf.waitcls = $(elems[i]).attr('waitcls') || opts.waitcls; //正在进行ajax验证时的类名
			conf.f_waitcls = $(elems[i]).attr('f_waitcls') || opts.f_waitcls; //正在进行ajax验证时的表单元素类名
			conf.waitmsg = $(elems[i]).attr('waitmsg') || opts.waitmsg; //正在进行ajax验证时的等待提示
			conf.emptymsg = $(elems[i]).attr('emptymsg') || opts.emptymsg; //为空时的提示信息
			conf.domcls = $(elems[i]).attr('domcls') || opts.domcls; //提示UI的类名
			conf.empty = $(elems[i]).attr('empty') || opts.empty; //是否允许为空
			conf.tiptype = $.trim($(elems[i]).attr('tiptype')) || opts.tiptype; //提示类型
			conf.ajaxurl = $(elems[i]).attr('ajaxurl') || ''; //ajax验证url
			conf.ajaxname = $(elems[i]).attr('ajaxname') || $(elems[i]).attr('name') || 'ajaxname'; //ajax验证时传送验证值的参数名
			...
			$(elems[i]).data('ev_conf',conf); //把值赋给元素,这里加个 前缀ev是为了防止和其他代码的冲突
			
			$(elems[i]).blur(onBlur).focus(onFocus);
		}
	...
	function specialValid(datatype){//特殊的验证规则
		...
		if(matches){//基本类型并且有数量限制的验证
			...
			verify = function(value,conf){
				if(value===''&&conf.empty){return null}//允许为空
	...
	function createValid(datatype,opts){//生成验证函数,验证函数必须返回一个状态码，为0或未定义时代表验证通过,为null时表示留空,为1时代表验证未通过,为2时代表得到焦点,为3时表示等待ajax返回结果,为4时表示ajax验证未通过
		if(/^\w+\(.*\)$/.test(datatype)){//自定义函数验证
	...
	function getTipdom(domcls,$elem,conf,form){//获取提示ui元素,只有在提示方式为2时才用得到
		if(/^\w+\(.*\)$/.test(domcls)){//自定义函数查找dom
			var f = opts.fn[domcls.replace(/\(.*\)$/,'')];
			if(f){
				var dom = f($elem,form);
				return $(dom).addClass(conf.domcls);//给自定义创建的dom元素添加配置中的类名
			}
		}
